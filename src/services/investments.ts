...
export const investmentsService = {
  ...
  async addRemark(
    ...
    return api.put(
      `/investments/${id}/${scheduleId}/update-specific-schedules-comments`,
      data
    );
  },
  ...
  async downloadDocument(id: string, documentId: string): Promise<Blob> {
    const response = await api.get(
      `/investments/${id}/documents/${documentId}/download`,
      {
        responseType: "blob",
      }
    );
  ...
  async getUpcomingDue(days?: number): Promise<
    ApiResponse<
      Array<{
        investmentId: string;
        investor: {
  ...
  async getPortfolioAnalytics(investorId: string): Promise<
    ApiResponse<{
      summary: {
        totalInvestments: number;
        totalValue: number;
  ...
  async getNotificationHistory(id: string): Promise<
    ApiResponse<
      Array<{
        id: string;
        type: string;
  ...
  async getRiskAnalysis(id: string): Promise<
    ApiResponse<{
      currentRisk: {
        score: number;
        level: string;
  ...
  > {
    return api.put("/investments/bulk", {
  ...
  async generatePortfolioReport(
    ...
    const response = await api.post(
      `/investors/${investorId}/portfolio-report`,
      options,
      {
        responseType: "blob",
      }
  ...
  async getComplianceStatus(id: string): Promise<
    ApiResponse<{
  ...
  async getAuditTrail(id: string): Promise<
    ApiResponse<
      Array<{
        timestamp: string;
        action: string;
  ...
  async getROIAnalysis(id: string): Promise<
    ApiResponse<{
      currentROI: number;
      projectedROI: number;
      benchmarkROI: number;
  ...
  > {
    return api.post(`/investments/${id}/performance-comparison`, {
      benchmarks,
    });
  },
  ...
  async getMaturityProjections(id: string): Promise<
    ApiResponse<{
      maturityDate: string;
  ...
  async getReinvestmentOptions(id: string): Promise<
    ApiResponse<{
      availablePlans: any[];
  ...
  async getCommunicationHistory(id: string): Promise<
    ApiResponse<{
      emails: any[];
      calls: any[];
      meetings: any[];
