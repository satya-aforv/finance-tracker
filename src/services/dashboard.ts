...
export const dashboardService = {
      ...
      try {
        ...
        return {
          ...
          data: {
            ...
            investorStats:
              stats.investorStats || this.getEmptyStats().investorStats,
            paymentStats:
              stats.paymentStats || this.getEmptyStats().paymentStats,
  ...
  async getDashboardUserOverview() {
    try {
      const response = await api.get("/dashboard/overview/user");
  ...
  > {
    try {
      ...
      const [investmentStats, investorStats, paymentStats, planStats] =
        await Promise.allSettled([
      ...
      return {
        ...
        data: {
          stats:
            investmentStats.status === "fulfilled" && investmentStats.value.data
              ? investmentStats.value.data
              : this.getEmptyStats().stats,
          investorStats:
            investorStats.status === "fulfilled" && investorStats.value.data
              ? investorStats.value.data
              : this.getEmptyStats().investorStats,
          paymentStats:
            paymentStats.status === "fulfilled" && paymentStats.value.data
              ? paymentStats.value.data
              : this.getEmptyStats().paymentStats,
          planStats:
            planStats.status === "fulfilled" && planStats.value.data
    ...
    try {
      ...
      if (response.data) {
        ...
        if (!data.investments?.chartData?.length) {
          data.investments.chartData = this.generatePlaceholderChartData(
            data.investments?.current || 0
          );
        }
        if (!data.payments?.chartData?.length) {
          data.payments.chartData = this.generatePlaceholderChartData(
            data.payments?.current || 0
          );
        }
        if (!data.investors?.chartData?.length) {
          data.investors.chartData = this.generatePlaceholderChartData(
            data.investors?.current || 0
          );
        }
        if (!data.revenue?.chartData?.length) {
          data.revenue.chartData = this.generatePlaceholderChartData(
  ...
  async getQuickActionsData(): Promise<
    ApiResponse<{
      pendingActions: {
  ...
  async getOverdueItems(): Promise<ApiResponse<any>> {
    try {
      ...
      return {
        success: true,
        data: { payments: [], documents: [], reviews: [] },
      };
    }
  },
