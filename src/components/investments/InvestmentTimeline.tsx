...
const InvestmentTimeline: React.FC<InvestmentTimelineProps> = ({
  ...
  const getRelativeTime = (date: string) => {
    ...
    const diffInMinutes = Math.floor(
      (now.getTime() - entryDate.getTime()) / (1000 * 60)
    );
            ...
            <AnimatePresence>
              {timeline.map((entry, index) => (
                ...
                >
                  {/* Timeline dot */}
                  <div
                    className={`relative z-10 flex items-center justify-center w-12 h-12 rounded-full ${getTimelineColor(
                      entry.type
                    )}`}
                  >
                          ...
                          <div className="text-xs text-gray-600">
                            {entry.metadata.oldStatus &&
                              entry.metadata.newStatus && (
                                <span>
                                  Status changed from{" "}
                                  <strong>{entry.metadata.oldStatus}</strong> to{" "}
                                  <strong>{entry.metadata.newStatus}</strong>
                                </span>
                              )}
                            {entry.metadata.fileName && (
                              <span>
                                File: <strong>{entry.metadata.fileName}</strong>
                              </span>
                            )}
                            {entry.metadata.category && (
                              <span>
                                Category:{" "}
                                <strong>{entry.metadata.category}</strong>
                              </span>
                            )}
                          </div>
                        </div>
          ...
          <div className="bg-blue-50 p-3 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-blue-800">
                This entry will be recorded as performed by{" "}
