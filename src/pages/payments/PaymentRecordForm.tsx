// src/pages/payments/PaymentRecordForm.tsx - Fixed with better error handling
...
const PaymentRecordForm: React.FC<PaymentRecordFormProps> = ({
  ...
  const [investmentData, setInvestmentData] = useState<any>(investment);
  ...
  useEffect(() => {
    const fetchInvestment = async (id = "687f12fd8c122c4487007e35") => {
      try {
        ...
        setPayment(
          response.data.schedule?.filter(
            (s) => s.month == paymentSchedule?.scheduleMonth
          )[0] || null
        );
    ...
    if (paymentSchedule?.investment?._id) {
      fetchInvestment(paymentSchedule?.investment?._id);
    }
  }, [paymentSchedule?.investment?._id]);

  useEffect(() => {
    if (investmentData) {
      setSelectedInvestment(investmentData || null);

      if (investmentData && investmentData.schedule) {
        // Get months that can accept payments (be more flexible)
        const availableScheduleMonths = investmentData.schedule
          .filter(
            (s) =>
  ...
  }, [watchInvestment, investmentData, setValue]);

  useEffect(() => {
    if (selectedInvestment && watchScheduleMonth) {
      const scheduleItem = availableMonths.find(
        (s) => s.month === watchScheduleMonth
      );
      if (scheduleItem) {
        ...
        setValue(
          "interestAmount",
          Math.min(remainingAmount, remainingInterest)
        );
        setValue(
          "principalAmount",
          Math.max(0, remainingAmount - remainingInterest)
        );
      }
    }
  ...
  useEffect(() => {
    ...
    if (watchPaymentMethod) {
      setValue(
        "documentCategory",
        suggestedCategories[watchPaymentMethod] as any
      );
    }
  ...
  const { required: requiredDocs, optional: optionalDocs } =
          ...
          <div>
            ...
            <input
              {...register("scheduleMonth", {
                required: "Schedule month is required",
              })}
              type="text"
              className="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="Enter month"
              value={`Month ${payment?.month} - ${formatCurrency(
                payment?.totalAmount
              )} remaining`}
              disabled
            />
            {errors.scheduleMonth && (
              <p className="mt-1 text-sm text-red-600">
                {errors.scheduleMonth.message}
              </p>
            )}
            {selectedInvestment && availableMonths.length === 0 && (
              <p className="mt-1 text-sm text-yellow-600">
                No pending payments found for this investment
              </p>
            )}
          </div>
      ...
      {payment && (
        <div className="bg-blue-50 p-4 rounded-lg">
          <h4 className="text-md font-medium text-blue-900 mb-3">
            Schedule Details - Month {payment.month}
          </h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="font-medium">
                {formatCurrency(scheduleDetails?.remainingAmount)}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <div className="mt-2 text-sm text-yellow-700">
                <div>
                  <strong>Required:</strong> {requiredDocs.join(", ")}
                </div>
                {optionalDocs.length > 0 && (
                  <div>
                    <strong>Optional:</strong> {optionalDocs.join(", ")}
                  </div>
                )}
              </div>
            </div>
